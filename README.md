...
| 指标 | 数值 | 说明 |
| --- | --- | --- |
| 自动化测试 | **630 个**（后端 234 · 小程序 152 · Web 244） | 每阶段改动全量回归，0 失败基线 |
| 小程序主包 | **854KB → 448KB（-47.5%）** | 构建期剔除 H5 专有产物 + `lazyCodeLoading` |
| 接口延迟 | 核心链路 **28~102ms** | 可用 `scripts/smoke_e2e.py` 现场复现 10 步全链路 |
| 离线能力 | 投递记录本机队列 | 4xx 拒绝入队、毒丸隔离、GET 幂等重试语义 |
