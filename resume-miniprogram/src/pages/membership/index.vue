...
async function load(): Promise<void> {
  ...
  error.value = ""
  try {
    const [status, items] = await Promise.all([getVipStatus(), listMembershipPackages()])
    vip.value = status
...
<template>
  <scroll-view class="page" scroll-y>
    <view class="status-card"><text class="eyebrow">MEMBERSHIP</text><text class="title">{{ vip?.vipLevel || "Loading" }} plan</text><text class="copy">{{ vip?.expireTime ? `Valid until ${vip.expireTime}` : "No active paid entitlement" }}</text></view>
