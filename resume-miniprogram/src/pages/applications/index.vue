...
import { deleteApplication, listApplications } from "../../services/application-api"
...
const visibleApplications = computed(() => filterApplications(applications.value, selectedStatus.value))
...
<template>
  <scroll-view class="page" scroll-y>
    <view class="content">
      <view class="hero">
        <text class="eyebrow">APPLICATION WORKSPACE</text>
      ...
      <view v-for="item in visibleApplications" :key="item.id" class="card record-card">
        ...
        <view class="actions"><button size="mini" class="secondary" @click="edit(item)">编辑</button><button size="mini" class="danger" @click="remove(item)">删除</button></view>
      </view>
    </view>
  </scroll-view>
</template>
...
.hero { padding: 30rpx 24rpx; background: linear-gradient(145deg,#e7f1ff,#f9fcff); border: 1rpx solid #d6e8ff; border-radius: 22rpx; }.eyebrow { display: block; color: #1677ff; font-size: 21rpx; font-weight: 700; letter-spacing: 1rpx; }.title { display: block; margin-top: 10rpx; font-size: 42rpx; font-weight: 700; }.subtitle { display: block; margin-top: 12rpx; color: #5f6f82; font-size: 24rpx; line-height: 1.65; }
...
@media (max-width: 360px) { .two-columns { grid-template-columns: 1fr; }.summary-card { align-items: flex-start; flex-direction: column; }.pending { align-items: flex-start; flex-direction: column; } }
</style>
