...
- Web resume saves that fail validation now expose the first existing field error through a form-level `role="alert"`, then focus and center the first invalid control in form order. Button, form-submit, and `Ctrl/Cmd+S` paths continue to call the same manual-save function; no remote autosave was added.
...
- Final focused verification passed: H5 26/26 tests and Web 38/38 tests. Final full verification passed: H5 117/117 tests across 42 files plus the H5 production build; Web 87/87 tests across 20 files plus the Web production build.
