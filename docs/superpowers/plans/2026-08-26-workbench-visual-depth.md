...
- [ ] **Step 1: Add light and dark `--shadow-hover` values beside existing shadow tokens.**
- [ ] **Step 2: Add a subtle sidebar shadow and `text-wrap: balance` for view headings.**
- [ ] **Step 3: Add pointer-only transitions and hover lift for valid high-value surfaces; keep invalid assessment cards excluded.**
- [ ] **Step 4: Extend the reduced-motion block to disable the new surface transitions.**
- [ ] **Step 5: Re-run `npm.cmd test -- src/tests/interaction.spec.ts` and confirm the contract passes.**
...
- [ ] **Step 1: Append the Web workspace visual polish entry without changing product copy.**
- [ ] **Step 2: Run `npm.cmd test` and `npm.cmd run build` from `web-frontend`.**
- [ ] **Step 3: Run `npm.cmd run test:unit` and `npm.cmd run build:h5` from `resume-miniprogram`.**
- [ ] **Step 4: Run `git diff --check` and the Impeccable detector over changed UI targets.**
- [ ] **Step 5: Start Web on an available port and verify the served title/script identify this project.**
- [ ] **Step 6: Commit the implementation and changelog, push the branch, and deploy the Web frontend to Vercel production.**
