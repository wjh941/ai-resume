...
- **R**：主包 854KB → 448KB（-47.5%），H5 产物与开发服务器不受影响；163 个单测全绿。
- **追问预案**：
...
- **R**：队列不会死循环，用户得到明确反馈而不是"一直在同步"；163 个单测覆盖全部路径（入队/拒入队/毒丸隔离/断网中止）。
- **追问预案**：
...
### 故事 4：Web 端首屏 113KB → 25.5KB 与缓存策略

- **S/T**：Web 端入口 chunk 113KB 里混着 Vue 运行时（30.8KB gzip）和 lucide 图标库（4.6KB gzip）——业务代码每次发版都会把这两块不常变的缓存一起作废；且视图按需分块后，用户第一次点导航要等整块下载。
- **A**：①`manualChunks` 把 `vendor-vue`/`vendor-icons` 拆为独立 chunk，入口降到 25.5KB（gzip 10.3KB）——发版只作废业务代码，框架与图标长期命中缓存；②视图组件在侧边栏 `pointerenter/focus` 时预热（`App.vue` 的 `prefetchView`，Set 去重 + 失败回退），点击时 chunk 已就绪，导航近乎零等待；③视图本体 `defineAsyncComponent` 带 120ms 延迟的加载骨架与 2 次重试，慢网不白屏。
- **R**：入口 gzip 43.4KB → 10.3KB（-76%）；重复访问只拉业务增量；653 测试全绿。
- **追问预案**：
  - *为什么不把 vendor 全部预加载？* 那等于放弃按需加载；拆分的意义正是让"必加载的框架"与"按需的视图"各自独立演进。
...
| 错误文案双层中文化 | 后端 62 处报错译为可行动中文且**错误码不变**；小程序按码映射兜底，校验类错误优先展示后端具体 detail | `main.py`、`services/http.ts` |
| 表单摩擦清零 | 手填 `YYYY-MM-DDTHH:mm:ss+08:00` 全部换原生日期/时间选择器（自动拼 ISO）；环形跳转用 `redirectTo` 防止微信 10 层页面栈塞满 | `pages/applications/index.vue`、`utils/navigation.ts` |
| 本地检查点 | 投递表单未保存内容防抖落盘（300ms），刷新后恢复条带回填，干净态即清除；与简历草稿检查点同一套防御式解析 | `lib/application-checkpoint.ts` |
| 三端调色板统一 | 小程序 131 处 AntD 蓝/混用红迁移到 Web 设计令牌（#2563eb/#1d4ed8/#bf3f3a），脚本化替换可复核 | `git show b2928dc` |
| 部署契约先演练 | 以 `VITE_API_BASE_URL` 构建 + 静态伺服 + CORS 白名单，在本地把 Vercel 会走的每一步（预检/认证请求/暴露头）验证过再上线 | `README.md` 部署概览 |
...
3. 打开 Web 工作台演示暗色模式持久化与侧边栏悬停预热（切视图无加载感）；小程序开发者工具导入 `dist/build/mp-weixin` 演示包体积（微信工具里可见主包 < 500KB）与四页 tabBar。
4. 断网点一次"保存投递"，恢复网络点"重试同步"——演示离线队列与跳过提示。

## 五、弱点与预案（主动说）

- **单人项目**：用流程回答——653 测试、每阶段强制全量回归、可重复冒烟脚本、提交按模块拆分，用工程纪律替代协作评审。
