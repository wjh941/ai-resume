...
describe("interaction contracts", () => {
  ...
  it("routes invalid resume saves and visible cancel controls through accessible shared handlers", () => {
    ...
    expect.soft(normalizedEditor).toMatch(/if \(result === "invalid"\) \{ const currentErrors = fieldErrors\.value \?\? \{\} activateInvalidSummary\(currentErrors\) await nextTick\(\) focusFirstInvalidResumeField\(currentErrors\) \}/)
  ...
  it("protects dirty application edits before leaving the tracker", () => {
    ...
    expect(applications).toContain("isFollowupDirty")
    ...
    expect(styles).toContain(".application-leave-confirmation-actions .compact")
  ...
  it("coordinates guarded workspace navigation around dirty applications", () => {
    ...
    expect(applications).toContain("let navigationBlocked = false")
    expect(normalizedApplications).toMatch(/navigationBlocked = true/)
    expect(normalizedApplications).toMatch(/function continueEditing\(\): void \{ navigationBlocked = false/)
    ...
    expect(normalizedApplications).toMatch(/const shouldResumeNavigation = navigationBlocked/)
    expect(normalizedApplications).toMatch(/if \(shouldResumeNavigation\) emit\("navigation-ready"\)/)
