...
<template>
  <section class="view-layout">
    <div class="view-heading"><div><h1>账户设置</h1><p>了解当前账户的数据范围，并在需要时完成隐私确认、导出或删除申请。</p></div></div>
    <p v-if="error" class="notice-error" role="alert">{{ error }}</p><p v-if="notice" class="notice-success" aria-live="polite">{{ notice }}</p>
    <div v-if="loading" class="content-skeleton" aria-busy="true"><span /><span /></div>
    <article v-else-if="scope" class="account-scope"><section><ShieldCheck :size="25" aria-hidden="true" /><div><h2>当前数据范围</h2><p>{{ scope.privacy_policy_hint }}</p></div></section><ul class="tag-list"><li v-for="category in scope.categories" :key="category">{{ category }}</li></ul><p class="source-notice">{{ scope.retention_note }}</p><div class="account-actions"><button class="text-action" type="button" @click="recordConsent"><ShieldCheck :size="16" aria-hidden="true" />确认隐私说明</button><button class="text-action" type="button" @click="prepareExport"><Download :size="16" aria-hidden="true" />准备数据导出</button><button class="danger-action" type="button" @click="requestDeletion"><Trash2 :size="16" aria-hidden="true" />申请删除账户</button></div></article>
  </section>
</template>
