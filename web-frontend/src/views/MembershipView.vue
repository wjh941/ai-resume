...
<template>
  <section class="view-layout membership-view">
    ...
    <template v-else>
      ...
      <section v-if="phase === 'awaiting-payment' && pendingOrder" class="payment-pending"><div><h2>订单待支付</h2><p>{{ pendingOrder.orderId }} · ¥{{ pendingOrder.totalAmount }} · 当前状态：{{ pendingOrder.paymentStatus }}</p><p v-if="!paymentEnabled" class="source-notice" role="status">{{ paymentNotice }}</p></div><AsyncButton class="primary-button compact" type="button" :disabled="!paymentEnabled || paymentMode !== 'demo'" @click="payDemo">{{ paymentMode === 'demo' ? "完成演示支付" : "完成支付" }}</AsyncButton></section>
