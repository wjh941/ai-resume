...
async function refresh(): Promise<void> {
  ...
  try { const [evidence, draftList] = await Promise.all([listEvidence(), listDrafts()]); items.value = evidence; resetVisibleEvidence(); drafts.value = draftList } catch { error.value = "暂时无法读取经历证据，请稍后重试" } finally { loading.value = false }
}
