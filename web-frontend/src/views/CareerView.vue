...
<template>
  <section class="view-layout">
    <div class="view-heading">
      <div><h1>职业规划</h1><p>把职业建议拆解为能在本周完成的小行动，并保留完成记录。</p></div>
    ...
    <form class="inline-form" @submit.prevent="addTask">
      ...
      <button class="primary-button compact" type="submit" :disabled="saving"><Plus :size="17" aria-hidden="true" />{{ saving ? "保存中" : "加入清单" }}</button>
    </form>
    ...
    <div v-else-if="tasks.length" class="task-list">
      <article v-for="task in tasks" :key="task.id" class="task-row" :class="{ 'is-complete': task.status === 'completed' }">
        <button class="task-check" type="button" :title="task.status === 'completed' ? '标记为未完成' : '标记为已完成'" :aria-label="task.status === 'completed' ? '标记为未完成' : '标记为已完成'" @click="toggleTask(task)"><Check :size="16" aria-hidden="true" /></button>
