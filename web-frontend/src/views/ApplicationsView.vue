...
<template>
  <section class="view-layout">
    ...
    <div class="application-toolbar"><label><span>筛选状态</span><select v-model="filterStatus" @change="refresh"><option value="">全部状态</option><option v-for="(label, value) in statusLabels" :key="value" :value="value">{{ label }}</option></select></label><span class="toolbar-hint">共 {{ items.length }} 条记录</span></div>
    ...
    <div v-else-if="items.length" class="application-table">
      <article v-for="item in items" :key="item.id" class="application-record">
        <div class="application-row"><span class="record-symbol record-sky"><Building2 :size="20" aria-hidden="true" /></span><div><h2>{{ item.roleName }}</h2><p>{{ item.company }}<template v-if="item.city"> · {{ item.city }}</template></p></div><select class="status-select" :value="item.status" :disabled="Boolean(pendingKey)" @change="handleStatusChange(item, $event)"><option v-for="(label, value) in statusLabels" :key="value" :value="value">{{ label }}</option></select><small>{{ item.nextInterviewAt || item.nextActionAt || "尚未设置下一步" }}</small><div class="record-actions"><AsyncButton class="text-action compact" type="button" @click="startEdit(item)"><Pencil :size="15" aria-hidden="true" />编辑</AsyncButton><AsyncButton class="text-action compact" type="button" :loading="pendingKey === `timeline-load:${item.id}`" @click="toggleTimeline(item)"><ChevronUp v-if="expandedId === item.id" :size="15" aria-hidden="true" /><ChevronDown v-else :size="15" aria-hidden="true" />时间线</AsyncButton><AsyncButton class="danger-action compact" type="button" :loading="pendingKey === `delete:${item.id}`" :disabled="Boolean(pendingKey)" @click="remove(item)"><Trash2 :size="15" aria-hidden="true" />删除</AsyncButton></div></div>
        <div v-if="expandedId === item.id" class="application-followup"><div class="followup-grid"><div><h3>跟进记录</h3><ol v-if="item.timeline.length" class="timeline-list"><li v-for="event in item.timeline" :key="event.id"><strong>{{ event.title }}</strong><small>{{ event.occurredAt }}</small><p v-if="event.description">{{ event.description }}</p></li></ol><p v-else class="source-notice">暂时没有时间线记录。</p></div><div class="followup-actions"><h3>添加跟进</h3><label><span>标题</span><input v-model.trim="timelineForm.title" maxlength="240" placeholder="例如：完成一面" /></label><label><span>时间</span><input v-model="timelineForm.occurredAt" type="datetime-local" /></label><label><span>说明</span><textarea v-model.trim="timelineForm.description" rows="2" maxlength="4000" /></label><AsyncButton class="primary-button compact" type="button" :loading="pendingKey === `timeline-add:${item.id}`" @click="addEvent(item)"><Clock3 :size="15" aria-hidden="true" />添加事件</AsyncButton><h3>设置提醒</h3><label><span>提醒时间</span><input v-model="reminderAt" type="datetime-local" /></label><AsyncButton class="text-action compact" type="button" :loading="pendingKey === `reminder:${item.id}`" @click="setReminder(item)"><CalendarClock :size="15" aria-hidden="true" />保存提醒</AsyncButton></div></div></div>
      </article>
    </div>
